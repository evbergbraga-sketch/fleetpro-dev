    ...
    dl.innerHTML = meusLocs.length ? meusLocs.slice(0,5).map(l=>{
      ...
      return `<tr style="cursor:pointer" onclick="goPage('locacoes')" title="Ver locações">
        <td><div style="display:flex;align-items:center;gap:8px">
          <div class="vi ${l.veiculos?.foto_url?'vi-foto':(l.veiculos?.tipo==='carro'?'vi-car':'vi-moto')}">${l.veiculos?.foto_url?`<img src="${l.veiculos.foto_url}" onerror="this.parentElement.className='vi ${l.veiculos?.tipo==='carro'?'vi-car':'vi-moto'}';this.parentElement.innerHTML=SVG_VEICULO('${l.veiculos?.tipo}')">`:SVG_VEICULO(l.veiculos?.tipo)}</div>
