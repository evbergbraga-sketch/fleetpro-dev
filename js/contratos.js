...
async function registrarContrato(retornarId=false){
  ...
  try{
    ...
    const {data:locSalva, error} = await _comRetry(() => sb.from('locacoes').insert({
      ...
      data_inicio_hora: _brISO(ini),
      data_fim_hora: _brISO(fim),
      km_inicial:km,
      diaria:d.dia,
...
async function calSelectDay(d){
  ...
  document.getElementById('cal-veic-list').innerHTML=allVeiculos.map(v=>{
    ...
    return `<div style="display:flex;align-items:center;justify-content:space-between;padding:10px;background:var(--bg3);border-radius:8px;border:1px solid var(--border)"><div style="display:flex;align-items:center;gap:8px"><div class="vi ${v.foto_url?'vi-foto':(v.tipo==='carro'?'vi-car':'vi-moto')}">${v.foto_url?`<img src="${v.foto_url}" onerror="this.parentElement.className='vi ${v.tipo==='carro'?'vi-car':'vi-moto'}';this.parentElement.innerHTML=SVG_VEICULO('${v.tipo}')">`:SVG_VEICULO(v.tipo)}</div><div><div style="font-size:13px;font-weight:500">${v.marca} ${v.modelo}</div><div style="font-size:11px;color:var(--muted)">${v.placa}</div>${dispInfo}</div></div><span class="badge ${b}">${lb}</span></div>`;
