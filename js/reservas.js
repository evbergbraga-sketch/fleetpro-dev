    ...
    return `<tr>
      ...
      <td>
        <div style="display:flex;align-items:center;gap:8px">
          <div class="vi ${veic?.foto_url?'vi-foto':(veic?.tipo==='carro'?'vi-car':'vi-moto')}">${veic?.foto_url?`<img src="${veic.foto_url}" onerror="this.parentElement.className='vi ${veic?.tipo==='carro'?'vi-car':'vi-moto'}';this.parentElement.innerHTML=SVG_VEICULO('${veic?.tipo}')">`:SVG_VEICULO(veic?.tipo)}</div>
          <div>
